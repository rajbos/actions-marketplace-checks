...
n    # Construct full blob URL: baseUrl + /actions.json + SAS query
    $blobUrl = "${baseUrl}/${script:actionsBlobFileName}${sasQuery}"
n    Write-Host "Blob URL: ${baseUrl}/${script:actionsBlobFileName} (SAS redacted)"

    try {
        Invoke-WebRequest -Uri $blobUrl -Method GET -OutFile $localFilePath -UseBasicParsing | Out-Null
n        if (Test-Path $localFilePath) {
...
n        [Parameter(Mandatory=$true)]
    ...
    $baseUrl = $baseUrlWithQuery.Substring(0, $queryStart)
    $sasQuery = $baseUrlWithQuery.Substring($queryStart)
n    # Construct full blob URL: baseUrl + /status/blobFileName + SAS query
    $blobUrl = "${baseUrl}/status/${blobFileName}${sasQuery}"
n    Write-Host "Blob URL: ${baseUrl}/status/$blobFileName (SAS redacted)"
...
n    # Download current version from blob storage to compare
...
n        if (Test-Path $tempCompareFile) {
            $remoteContent = [System.IO.File]::ReadAllBytes($tempCompareFile)
            $remoteFileSize = $remoteContent.Length
n            # Compare file content using SHA256 hash for efficiency (especially for large files)
            $filesMatch = $false
...
n            Write-Host "Changes detected in $blobFileName (local: $localFileSize bytes, remote: $remoteFileSize bytes)"
        }
        else {
...
n        $response = Invoke-WebRequest -Uri $blobUrl -Method PUT -Body $localContent -Headers $headers -UseBasicParsing
n        if ($response.StatusCode -eq 201 -or $response.StatusCode -eq 200) {
...
n    # Validate that access token is not null or empty before making API calls
...
n    $seconds = [math]::Round($totalSeconds, 0)
...
n    Write-Message -message "**${title}:**" -logToSummary $true
...
n    $waitTime = Format-WaitTime -totalSeconds $waitSeconds
n    Write-Message -message "" -logToSummary $true
...
n    # Call the rate_limit API to get token expiration information from response headers
...
n        # Check for token expiration header
...
n    # Check if the token is about to expire within the specified number of minutes
    # Returns $true if expiration is imminent, $false otherwise
n    $currentTime = [DateTime]::UtcNow
    $timeRemaining = $expirationTime - $currentTime
n    if ($timeRemaining.TotalMinutes -le $warningMinutes) {
        return $true
    }
n    return $false
}
...
n    if ($null -ne $existingForks -and $existingForks.Count -gt 0) {
...
n    # Validate that accessToken is not null or empty
...
n    # Validate that access_token_destination is not null or empty
...
n    #store the given access token as the environment variable GITHUB_TOKEN so that it will be used in the Workflow run
    $env:GITHUB_TOKEN = $accessToken
n    Write-Host "Got an access token with a length of [$($accessToken.Length)], running for [$($numberOfReposToDo)] repos"
...
n    # Always write to console
    Write-Host $message
n    # Store message in buffer
    [void]$buffer.Messages.Add($message)
n    # Track if this is an error
    if ($isError) {
...
n    # Get the latest commit SHA from the default branch of a repository using the GitHub API
...
n    if ([string]::IsNullOrWhiteSpace($owner) -or [string]::IsNullOrWhiteSpace($repo)) {
        return @{
            success = $false
            sha = $null
            branch = $null
...
n        if ($null -eq $repoInfo -or $repoInfo -eq $false) {
            return @{
                success = $false
                sha = $null
                branch = $null
...
n        $defaultBranch = $repoInfo.default_branch
        if ([string]::IsNullOrWhiteSpace($defaultBranch)) {
            # Repository metadata may not have default_branch set; default to main
            $defaultBranch = "main"
        }
n        # Get the latest commit from the default branch
...
n        if ($null -eq $branchInfo -or $branchInfo -eq $false) {
...
n    # Compare the latest commit hashes of source and mirror repositories
...
n    Write-Debug "Comparing commits: source [$sourceOwner/$sourceRepo] vs mirror [$mirrorOwner/$mirrorRepo]"
n    # Get source repository commit
...
n    # Get mirror repository commitn    $mirrorCommit = Get-RepositoryDefaultBranchCommit -owner $mirrorOwner -repo $mirrorRepo -access_token $access_token
...
n    # Check if a repository exists using the GitHub API
    # Returns $true if the repo exists and is accessible, $false otherwise
n    if ([string]::IsNullOrWhiteSpace($owner) -or [string]::IsNullOrWhiteSpace($repo)) {
        return $false
    }
n    $url = "repos/$owner/$repo"
    try {
...
n    # Execute a git command with exponential backoff retry logic
...
n            # Check if this is a transient error that should be retried
            $outputString = $output | Out-String
            $isTransientError = $outputString -like "*could not read Username*" -orn                               $outputString -like "*Connection refused*" -or
                               $outputString -like "*Connection timed out*" -or
                               $outputString -like "*SSL*" -or
                               $outputString -like "*Network is unreachable*"
n            if (-not $isTransientError) {
...
n            $lastError = $outputString
            $lastOutput = $output
n            if ($attempt -lt $MaxRetries) {
...
n    # Sync a mirror repository by pulling from upstream and pushing to mirror
...
n    Write-Debug "Syncing mirror [$owner/$repo] with upstream [$upstreamOwner/$upstreamRepo]"
n    # Validate parameters
...
n    # Check if upstream repository exists before attempting sync
...
n    # Check if mirror repository exists
...
n    # Early sync detection: Compare commit hashes before cloning
...
n    if ($comparison.can_compare -and $comparison.in_sync) {
...
n    # If comparison failed, continue with normal sync process (clone/fetch/merge)
...
n        # Clone the mirror repo with retry logic
...
n        Set-Location $repo | Out-Null
n        # Configure git user identity locally for this repo only
...
n        # Add upstream remote with authentication to avoid rate limiting
...
n        # Fetch from upstream with retry logic
...
n        # Check if upstream has the target branch using explicit refs
...
n            # Get the upstream repository's default branch
...
n            # If we found the upstream's default branch and it's different, use it
            if ($null -ne $upstreamDefaultBranch -and $upstreamDefaultBranch -ne $currentBranch) {
                ...
                $upstreamBranchRef = "refs/remotes/upstream/$currentBranch"
n                # Verify the upstream branch exists in our fetched refs before proceeding
...
n                # Force the mirror to use upstream's default branch
...
n        # Check if the mirror repo is empty (no commits yet)
...
n        # Flag to track if we need to force push (e.g., after conflict resolution or branch mismatch)
...
n        # If repo is empty, do an initial sync from upstream instead of merge
        if ($isEmptyRepo) {
...
n            $afterHash = git rev-parse HEAD 2>&1
            if ($LASTEXITCODE -ne 0) {
...
n                    # Force update the mirror to match upstream (upstream is always correct)
...
n                    Write-Debug "Successfully force updated mirror to match upstream after conflict"
                    $needForcePush = $true
                }
                elseif ($mergeOutput -like "*refusing to merge unrelated histories*") {
                    ...
                    git merge --abort 2>&1 | Out-Null
n                    Write-Warning "Unrelated histories detected. Force updating mirror to match upstream."
                    $resetRef = "refs/remotes/upstream/$currentBranch"
                    $resetResult = git reset --hard $resetRef 2>&1
n                    if ($LASTEXITCODE -ne 0) {
                        $resetOutput = $resetResult | Out-String
...
n                    Write-Debug "Successfully force updated mirror to match upstream after unrelated histories"
                    $needForcePush = $true
                }
...
n        # Disable GitHub Actions before pushing changes to prevent workflows from running
...
n        # Push changes back to mirror using explicit branch reference with retry
...
n        Write-Debug "Successfully synced mirror [$owner/$repo]"
n        # Clean up
        Set-Location $originalDir | Out-Null
        Remove-Item -Path $syncTempDir -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
n        # Clear LFS skip smudge environment variable
        Remove-Item Env:\GIT_LFS_SKIP_SMUDGE -ErrorAction SilentlyContinue
n        # Return appropriate message based on whether this was an initial sync, merge, or force update
        if ($isEmptyRepo) {
...
n        # Categorize errors for better reporting
        $errorType = "unknown"
        $cleanMessage = $errorMessage
n        if ($errorMessage -like "*Merge conflict*") {
...
n    # Disable GitHub Actions for a repository to prevent workflows from running on push
...
n    $url = "repos/$owner/$repo/actions/permissions"
    $body = @{
        enabled = $false
    } | ConvertTo-Json
n    Write-Debug "Disabling GitHub Actions for [$owner/$repo]"
n    try {
...
n    .PARAMETER existingForks
    The full list of forks from status.json
n    .PARAMETER numberOfChunks
...
n    .PARAMETER filterToUnprocessed
...
n    Write-Message -message "Splitting actions into [$numberOfChunks] chunks for parallel processing" -logToSummary $true
n    $actionsToProcess = $actions
n    # Optionally filter to actions with repoUrl
    if ($filterToUnprocessed) {
        $actionsToProcess = $actions | Where-Object {n            $null -ne $_.repoUrl -and $_.repoUrl -ne ""
        }
    }
n    if ($actionsToProcess.Count -eq 0) {
        Write-Message -message "No actions to process" -logToSummary $true
        return @{}
    }
n    Write-Message -message "Found [$($actionsToProcess.Count)] actions to process out of [$($actions.Count)] total" -logToSummary $true
n    # Calculate chunk size (round up to ensure all items are included)
...
n        if ($startIndex -lt $actionsToProcess.Count) {
            # Get the subset of action names for this chunk
            $chunkActions = @()
            for ($j = $startIndex; $j -le $endIndex; $j++) {
                $action = $actionsToProcess[$j]
                $identifier = $null
n                # Try forkedRepoName first (used for actions from actions.json)
                ...
                # Fall back to computing from RepoUrl or repoUrl (used for marketplace actions)
                    ...
                    $repoUrlValue = if ($null -ne $action.RepoUrl -and $action.RepoUrl -ne "") { $action.RepoUrl }n                                   elseif ($null -ne $action.repoUrl -and $action.repoUrl -ne "") { $action.repoUrl }n                                   else { $null }
n                    if ($null -ne $repoUrlValue) {
...
n    .PARAMETER numberOfRepos
    Maximum number of repos to select for processing
n    .PARAMETER coolOffHoursForFailedSync
...
n    Write-Message -message "Selecting up to [$numberOfRepos] forks to process" -logToSummary $true
    Write-Message -message "Cool-off period for failed syncs: [$coolOffHoursForFailedSync] hours" -logToSummary $true
n    # Filter forks that are eligible for processing
    $eligibleForks = $existingForks | Where-Object {
        # Must have a mirror
        if ($_.mirrorFound -ne $true) {
            return $false
        }
n        # Skip if upstream is marked as unavailable
        if ($_.upstreamAvailable -eq $false) {
            return $false
        }
n        # Check cool-off period for failed syncs
...
n    Write-Message -message "Found [$($eligibleForks.Count)] eligible forks after filtering" -logToSummary $true
n    # Sort by priority score (higher score = higher priority)
    # Priority considers:
    ...
    $sortedForks = $eligibleForks | Sort-Object -Property {
        $priorityScore = 0
        $lastSyncDate = $null
n        # Base priority: days since last successful sync
...
n        # Penalty: if repo had a recent failure, subtract penalty to deprioritize
...
n                # Determine if this is a repeated failure:
...
n                if ($isRepeatedFailure -and $hoursSinceAttempt -lt 168) {  # Within last 7 days
...
n        return $priorityScore
    } -Descending
n    # Select the top N repos (highest priority scores)
    $selectedForks = $sortedForks | Select-Object -First $numberOfRepos
n    # Calculate statistics about selection
    $reposWithRecentFailures = ($eligibleForks | Where-Object {n        if ($_.lastSyncError -and $_.lastSyncAttempt -and $_.lastSynced) {
            try {
                $lastAttemptDate = [DateTime]::Parse($_.lastSyncAttempt)
                $lastSyncDate = [DateTime]::Parse($_.lastSynced)
...
n    Write-Message -message "Selected [$($selectedForks.Count)] forks for processing" -logToSummary $true
...
n    Write-Message -message "Splitting forks into [$numberOfChunks] chunks for parallel processing" -logToSummary $true
n    # Filter to only forks that should be processed (mirrorFound = true)
    $forksToProcess = $existingForks | Where-Object { $_.mirrorFound -eq $true }
n    if ($forksToProcess.Count -eq 0) {
        Write-Message -message "No forks to process (all have mirrorFound = false)" -logToSummary $true
        return @{}
    }
n    Write-Message -message "Found [$($forksToProcess.Count)] forks to process out of [$($existingForks.Count)] total" -logToSummary $true
n    # Calculate chunk size (round up to ensure all items are included)
...
n    Write-Message -message "Saving partial status update for chunk [$chunkId] to [$outputPath]" -logToSummary $true
n    if ($null -eq $processedForks -or $processedForks.Count -eq 0) {
...
n    Write-Message -message "Saving [$($processedForks.Count)] processed forks for chunk [$chunkId]" -logToSummary $true
n    # Convert to JSON and save
    $json = ConvertTo-Json -InputObject $processedForks -Depth 10
    [System.IO.File]::WriteAllText($outputPath, $json, [System.Text.Encoding]::UTF8)
n    Write-Message -message "✓ Saved partial status for chunk [$chunkId]" -logToSummary $true
    return $true
}
...
n    Write-Host "Saving chunk [$chunkId] summary to [$outputPath]"
n    # Convert to JSON and save
    $json = ConvertTo-Json -InputObject $summary -Depth 5
    [System.IO.File]::WriteAllText($outputPath, $json, [System.Text.Encoding]::UTF8)
n    Write-Host "✓ Saved chunk summary for chunk [$chunkId]"
    return $true
}
...
n    Write-Message -message "# Overall Chunk Processing Summary" -logToSummary $true
    Write-Message -message "" -logToSummary $true
n    # Initialize totals
    $totalSynced = 0
    $totalUpToDate = 0
    ...
    $totalSkipped = 0
    $totalProcessed = 0
n    # Initialize failure breakdown tracking
    $failuresByType = @{}
    $allFailedRepos = @()
n    if ($null -eq $chunkSummaryFiles -or $chunkSummaryFiles.Count -eq 0) {
...
n            $totalSynced += $chunkSummary.synced
...
n            # Collect failed repos if available
...
n                    # Count by error type for breakdown
                    $errorType = $failedRepo.errorType
                    if ([string]::IsNullOrEmpty($errorType)) {
                        $errorType = "unknown"
                    }
n                    if ($failuresByType.ContainsKey($errorType)) {
...
n            Write-Host "  Chunk [$($chunkSummary.chunkId)]: Processed $($chunkSummary.totalProcessed) repos"
        }
        catch {
...
n    # Display consolidated summary
    ...
    Write-Message -message "" -logToSummary $true
n    # Display failure breakdown if there are failures
    if ($failuresByType.Count -gt 0) {
        ...
        Write-Message -message "|------------|------:|" -logToSummary $true
n        # Sort by count descending for better visibility
...
n    # Display first 10 failed repos with clickable links in a collapsible section
...
n            # Create clickable GitHub link using the configured fork organization
            $repoLink = "[$repoName](https://github.com/$forkOrg/$repoName)"
n            # Truncate error message if too long
            if ($errorMessage -and $errorMessage.Length -gt 100) {
                $errorMessage = $errorMessage.Substring(0, 97) + "..."
            }
n            Write-Message -message "| $repoLink | $errorType | $errorMessage |" -logToSummary $true
        }
n        Write-Message -message "" -logToSummary $true
        Write-Message -message "</details>" -logToSummary $true
        Write-Message -message "" -logToSummary $true
...
n    .PARAMETER currentStatus
    The current full status array from status.json
n    .PARAMETER partialStatusFiles
...
n    Write-Message -message "Merging partial status updates from [$($partialStatusFiles.Count)] chunks" -logToSummary $true
n    # Create a hashtable for fast lookup by name
    $statusByName = @{}
    foreach ($item in $currentStatus) {
        $statusByName[$item.name] = $item
    }
n    $totalUpdates = 0
n    foreach ($partialFile in $partialStatusFiles) {
        if (-not (Test-Path $partialFile)) {
            Write-Warning "Partial status file not found: [$partialFile]"
            continue
        }
n        Write-Host "Processing partial status file: [$partialFile]"
n        try {
...
n            if ($null -eq $partialStatus -or $partialStatus.Count -eq 0) {
                Write-Host "  No updates in this chunk"
                continue
            }
n            Write-Host "  Found [$($partialStatus.Count)] updates in this chunk"
n            # Merge each updated fork from the partial status into the current status
...
n                    $updatedFork.PSObject.Properties | ForEach-Object {
                        $propName = $_.Name
                        $propValue = $_.Value
n                        if (Get-Member -InputObject $existing -Name $propName -MemberType Properties) {
...
n    Write-Message -message "✓ Merged [$totalUpdates] fork updates from [$($partialStatusFiles.Count)] chunks into main status" -logToSummary $true
n    # Convert hashtable back to array
...
n    Write-Message -message "" -logToSummary $true
    ...
    Write-Message -message "" -logToSummary $true
n    # Calculate 7-day window
    $sevenDaysAgo = (Get-Date).AddDays(-7)
n    # Total repos in dataset
    $totalRepos = $existingForks.Count
n    # Count repos with mirrorFound = true (valid mirrors)
    $reposWithMirrors = ($existingForks | Where-Object { $_.mirrorFound -eq $true }).Count
n    # Count repos synced in the last 7 days
    $reposSyncedLast7Days = ($existingForks | Where-Object {n        if ($_.lastSynced) {
            try {
...
n    # Count repos with valid mirrors but no lastSynced timestamp
    $reposNeverSynced = ($existingForks | Where-Object {n        $_.mirrorFound -eq $true -and [string]::IsNullOrEmpty($_.lastSynced)
    }).Count
n    # Calculate percentages
...
n    $reposNotChecked = $reposWithMirrors - $reposSyncedLast7Days
n    Write-Message -message "**Total Repositories in Dataset:** $totalRepos" -logToSummary $true
